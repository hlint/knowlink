...
export async function actionUpdateIllustration(name: string) {
  ...
  (async () => {
    const url = await generateIllustration(name);
...
export async function actionGetIllustration(name: string) {
  await checkUserAdmin();
  const illustration = await prisma.illustration.findFirst({
    where: { name },
  });
  if (!illustration) {
    return await actionUpdateIllustration(name);
  }
  return illustration;
}
