...
export async function generateIllustration(categoryInfo: string) {
  const description = await callLlm({
    dialog: [
      { role: "system", content: prompt },
      { role: "user", content: `<INFORMATION>${categoryInfo}</INFORMATION>` },
    ],
  });
...
const prompt = `You are a professional illustrator. Create a specific scene description for a landscape header image based on the given INFORMATION.

Requirements:
- Create a single, specific scene that related to the INFORMATION
